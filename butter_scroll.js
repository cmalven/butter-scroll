(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ButterScroll"] = factory();
	else
		root["ButterScroll"] = factory();
})(this, function() {
...
module.exports = function (options) {
  ...
  */
  ...
  var _addDepthItems = function _addDepthItems() {
  ...
  var _addEventListeners = function _addEventListeners() {
    $(window).on('resize', throttle(_onResize, self.eventThrottleMs)).on('scroll', throttle(_onScroll, self.eventThrottleMs)).trigger('resize');
  };
  ...
  var _updateDepth = function _updateDepth() {
    var depthItemsLength = self.depthItems.length;
    while (depthItemsLength--) {
      var item = self.depthItems[depthItemsLength];
  ...
  var _completeDepthOffsets = function _completeDepthOffsets() {
    var depthItemsLength = self.depthItems.length;
    while (depthItemsLength--) {
      var item = self.depthItems[depthItemsLength];
      item.currentOffset = item.targetOffset;
  ...
  var _setDepthItemOffset = function _setDepthItemOffset(item, offset) {
    item.$el[0].style[self.prefixedTransform] = 'translateY(' + offset + 'px) translateZ(0)';
  };

  var _onResize = function _onResize(evt) {
    var height = self.settings.$elToScroll.innerHeight();
    $('.scroller').height(height);
  };

  var _onScroll = function _onScroll(evt) {
